...
function generateCarCard(data) {
    const activeBook = data.pesanan.filter(
        (entry) => entry["status"] == "belum_selesai"
    );
    const carIsAvailable =
        data.status == "tersedia" &&
        (data.pesanan.length == 0 || activeBook.length == 0);

    const div0 = document.createElement("div");
    ["col-md-6", "col-lg-4", "col-xl-3", "pt-4"].forEach((elemClass) =>
        div0.classList.add(elemClass)
    );

    const div1 = document.createElement("div");
    ["card", "shadow", "bg-white", "card-container"].forEach((elemClass) =>
        div1.classList.add(elemClass)
    );

    const div2_0 = document.createElement("div");
    if (carIsAvailable) {
        div2_0.textContent = "Tersedia";
        ["card-header", "text-center", "bg-success", "text-white"].forEach(
            (elemClass) => div2_0.classList.add(elemClass)
        );
    } else {
        div2_0.textContent =
            activeBook.length == 0
        ...
        ["card-header", "text-center", "bg-danger", "text-white"].forEach(
            (elemClass) => div2_0.classList.add(elemClass)
        );
    }

    const div2_1 = document.createElement("div");
    ["overlay", "rounded-1", "shadow"].forEach((elemClass) =>
        div2_1.classList.add(elemClass)
    );
    ...
    ["card-img-top", "rounded-0", "card-image"].forEach((elemClass) =>
        img.classList.add(elemClass)
    );

    const div2_2 = document.createElement("div");
    ["p-3", "card-content"].forEach((elemClass) =>
        div2_2.classList.add(elemClass)
    );

    const h5 = document.createElement("h5");
    h5.innerHTML = `${data.brand} <br> ${data.model}`;
    ["card-title", "fw-semibold"].forEach((elemClass) =>
        h5.classList.add(elemClass)
    );
    ...
    ["fa-solid", "fa-users"].forEach((elemClass) =>
        i5_0.classList.add(elemClass)
    );
    ...
    ["fa-solid", "fa-gear"].forEach((elemClass) =>
        i5_1.classList.add(elemClass)
    );
    ...
    ["fa-solid", "fa-gas-pump"].forEach((elemClass) =>
        i5_2.classList.add(elemClass)
    );
    ...
    if (carIsAvailable) {
        ...
        ["btn", "button-36"].forEach((elemClass) =>
            button.classList.add(elemClass)
        );
    } else {
        ["btn", "button-36-disabled", "text-white-50"].forEach((elemClass) =>
            button.classList.add(elemClass)
        );
    }
    ...
    [h5, p3_0, div3_0, div3_1, div3_2, div3_3].forEach((elem) =>
        div2_2.appendChild(elem)
    );

    [div2_0, div2_1, img, div2_2].forEach((elem) => {
    ...
    if (endDate < startDate) {
        ...
        await fetch("/api/mobilFilter", {
            ...
            body: Object.entries({
                ...
                transmission:
                    transmission == "anyTransmission" ? null : transmission,
                ...
                startDate:
                    startDateInput.value == ""
                        ? null
                        : convertDateFormatYMD(startDateInput.value),
                endDate:
                    endDateInput.value == ""
                        ? null
                        : convertDateFormatYMD(endDateInput.value),
            })
                .map(([key, value]) =>
                    value == null
                        ? null
            ...
            .then((res) => {
                ...
                Object.entries(res).forEach(([_, data]) => {
                    data["transmisi"] =
                        data["transmisi"][0].toUpperCase() +
                        data["transmisi"].substring(1);
                    data["bahan_bakar"] =
                        data["bahan_bakar"][0].toUpperCase() +
                        data["bahan_bakar"].substring(1);
